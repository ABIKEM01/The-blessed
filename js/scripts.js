...
function ready(){
    ...
    const addToCartButtons = document.getElementsByClassName('collection-items-btn')
    for(let i =0; i<addToCartButtons.length; i++){
        const addToCartButton = addToCartButtons[i]
        addToCartButton.addEventListener('click', addToCartClicked)
    } 

     document.getElementsByClassName('cart-item-purshase')[0].addEventListener('click', purchasedButtonClicked)
...
function purchasedButtonClicked(e){
    ...
    if(!tbody.hasChildNodes()){
        ...
        alert('Items purchased, Thanks for your patronage.')
        tbody.innerHTML = ''
        updateCartTotal()
    }
...
function addToCartClicked(e){
     ...
     const title = buttonParent.getElementsByClassName('collection-items-title')[0].textContent
     const price =  buttonParent.getElementsByClassName('collection-items-price')[0].textContent
...
function addToCart(title, price, imgSrc){
    ...
    for(let i=0; i<cartItems.length; i++){
        ...
        const cartTitle = cartItem.getElementsByClassName('cart-title')[0].textContent
        log(cartTitle)
        if(cartTitle == title){
            alert('Item already added, Kindly select other')
            return
        }
    }
    ...
    const cartItems = cart.getElementsByClassName('cart-items')
      let sum = 0
        for(let i=0; i<cartItems.length; i++){
                ...
                const quantityElement = cartItem.getElementsByClassName('cart-items-qty')[0]
